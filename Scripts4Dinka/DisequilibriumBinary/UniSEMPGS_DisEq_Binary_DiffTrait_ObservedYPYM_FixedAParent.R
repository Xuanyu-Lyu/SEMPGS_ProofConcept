...
fitUniSEMPGS_DisEq_Binary_DiffTrait_ObservedYPYM_FixedAParent <- function(data_path, h2_RDR_parent, h2_RDR_offspring, feaTol = 1e-6, optTol = 1e-8, jitterMean = .2, jitterVar = .05, extraTries = 30, exhaustive = F){
    library(OpenMx)
    library(data.table)
    library(stringr)
    ...
    Model1 <- mxModel("UniSEM_Binary_NonEquilibrium_DiffTrait_Observed_FixedAParent", Params, mxData(observed=Example_Data, type="raw"))
