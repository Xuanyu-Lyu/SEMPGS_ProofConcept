...
fitUniSEMPGS_SameTrait_ObservedYPYM <- function(data_path, feaTol = 1e-6, optTol = 1e-8, jitterMean = .5, jitterVar = .1, extraTries = 30, exhaustive = F){
    ...
    itlo  <- mxMatrix(type="Full", nrow=1, ncol=1, free=T, values=.05, label="itlo11", name="itlo") 
    itol  <- mxMatrix(type="Full", nrow=1, ncol=1, free=T, values=.05, label="itol11", name="itol")
