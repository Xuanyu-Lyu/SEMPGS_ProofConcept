...
## To identify this model, the optimal solution is to use RDR to find 'a' for offspring and use the rest of info to estimate 'a' for parents.

fitUniSEMPGS_DisEq_DiffTrait_ObservedYPYM_EstimatedAParent <- function(data_path, h2_RDR_offspring, feaTol = 1e-6, optTol = 1e-8, jitterMean = .5, jitterVar = .1, extraTries = 30, exhaustive = F){
    library(OpenMx)
    library(data.table)
    library(stringr)
    ...
    VY_p  <- mxMatrix(type="Full", nrow=1, ncol=1, free=T, values=1.5,   label="VY_parent", name="VY_p", lbound = .001)
    VY_o  <- mxMatrix(type="Full", nrow=1, ncol=1, free=T, values=1.5,   label="VY_offspring", name="VY_o", lbound = .001)
    ...
    Model1 <- mxModel("UniSEM_NonEquilibrium_DiffTrait_Observed_RDR", Params, mxData(observed=Example_Data, type="raw"))
